...
(function(){
  ...
  data.careerV7.linkedinUrl=data.careerV7.linkedinUrl||'';
  ...
  function jobPanel(){return `${section('Live Opportunities','Current openings with direct links')}<div class="featured-job-grid">${featuredJobs.map((j,i)=>`<article class="card featured-job"><div class="featured-job-top"><div><span class="job-company">${esc(j.company)}</span><h3>${esc(j.role)}</h3><p>${esc(j.location)} • ${esc(j.compensation)}</p></div><div class="job-fit"><b>${j.fit}%</b><small>FIT</small></div></div><p class="job-why">${esc(j.why)}</p><div class="job-actions"><a class="btn gold" href="${j.link}" target="_blank" rel="noopener">View Job</a><button class="btn" onclick="trackFeaturedJob(${i})">Add to Pipeline</button></div></article>`).join('')}</div><div class="notice">Job links are current search results and may close or change. Review the employer posting before applying.</div>`}
  }
